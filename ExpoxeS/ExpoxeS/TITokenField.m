...
- (void)setup {
    ...
    resultsObjectArray = [[NSMutableArray alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	id representedObject = [resultsArray objectAtIndex:indexPath.row];
    TIToken * token = [[TIToken alloc] initWithTitle:[self displayStringForRepresentedObject:representedObject]  representedObject:representedObject pfobject:[resultsObjectArray objectAtIndex:indexPath.row]];
...
- (void)resultsForSearchString:(NSString *)searchString {
	...
	[resultsArray removeAllObjects];
    [resultsObjectArray removeAllObjects];
	[resultsTable reloadData];
	...
	if (searchString.length)
    {
		...
		[resultsTable reloadData];*/
        //NSArray *result = [Brands listOfBrands:searchString token:tokenField.tokens];
        [Brands listOfBrands:searchString token:tokenField.tokens target:self];
    }
}

-(void)callBack:(NSArray*)result 
{
    for(PFObject *object in result)
    {
        [resultsObjectArray addObject:object];
        [resultsArray addObject:[object objectForKey:@"name"]];
    }
    [resultsTable reloadData];
...
{
	return [self initWithTitle:aTitle representedObject:object font:[UIFont systemFontOfSize:14] pfobject:nil];
}

-(id)initWithTitle:(NSString *)aTitle representedObject:(id)object pfobject:(PFObject *)pfobject
{
    return [self initWithTitle:aTitle representedObject:object font:[UIFont systemFontOfSize:14] pfobject:pfobject];
}

- (id)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont;
{
    return [self initWithTitle:aTitle representedObject:object font:aFont pfobject:nil];
}


- (id)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont pfobject:(PFObject*)pfobject {
	...
	if ((self = [super init])){
        ...
        localPFObject = [pfobject retain];
        thePFObject = pfobject;
